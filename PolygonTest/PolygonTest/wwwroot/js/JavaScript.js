const svg = SVG().addTo("#drawing-block").size(300, 200);
...
function sendData(data) {
    ...
    xhr.open("POST", "/polygons/data", true);       //путь должен совпадать с названием контроллера
...
function drawPoly(e) {
    ...
    if (points.length > 0) {
        ...
        svg.line(prev.x, prev.y, x, y).stroke({ color: "#000", width: 2 });
    }
    points.push({ x: x, y: y });
}
